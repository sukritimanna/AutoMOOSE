...
def _save(run_id: str):
    ...
    # also write metadata.json (human-readable subset)
...
def _run_thread(run_id: str, physics: str, params: dict, mpi: int):
    ...
    # build human-readable run dir name
    ...
    # no executable → input_ready mode
...
def _launch_sweep(physics: str, sweep_param: str, values: list,
    ...
    for v in values:
        ...
        _runs[run_id] = {"run_id":run_id,"status":"pending","physics":physics,
            "params":params,"metrics":{},"sweep":sweep_param}
        _save(run_id)
...
async def get_plugins():
    ...
    for pid, p in registry.all_plugins().items():
        ...
        out[pid] = {
            ...
            "sweepable":p.get("sweepable",[]),"result_keys":p.get("result_keys",[]),
        }
    return out
...
async def generate_only(req: RunRequest):
    ...
    if plugin["status"]=="stub": raise HTTPException(400,f"Plugin '{req.physics}' not implemented.")
...
async def start_run(req: RunRequest, background_tasks: BackgroundTasks):
    ...
    run_id = make_run_id()
    params = {**plugin.get("params",{}), **req.params}
    _runs[run_id] = {"run_id":run_id,"status":"pending","physics":req.physics,
